<section class="page-header">
    <div>
        <h1><?= e($title) ?></h1>
        <p class="page-subtitle">Cadastro base usado por anamnese, diario, agenda e financeiro.</p>
    </div>
...
<section class="panel">
    <form class="form-grid" method="POST" action="<?= url($action) ?>">
        ...
        <label>
            ...
            <select name="responsavel_id">
                ...
                <?php foreach ($responsaveis as $responsavel): ?>
                    <option value="<?= (int) $responsavel['id'] ?>" <?= (string) ($paciente['responsavel_id'] ?? '') === (string) $responsavel['id'] ? 'selected' : '' ?>>
        ...
        <label>
            ...
            <select name="cuidador_id">
                ...
                <?php foreach ($cuidadores as $cuidador): ?>
                    <option value="<?= (int) $cuidador['id'] ?>" <?= (string) ($paciente['cuidador_id'] ?? '') === (string) $cuidador['id'] ? 'selected' : '' ?>>
        ...
        <label>
            Status
            <select name="status">
                <option value="Ativo" <?= ($paciente['status'] ?? 'Ativo') === 'Ativo' ? 'selected' : '' ?>>Ativo</option>
                <option value="Inativo" <?= ($paciente['status'] ?? '') === 'Inativo' ? 'selected' : '' ?>>Inativo</option>
            </select>
        ...
        <div class="form-actions span-2">
            <button class="btn btn-primary" type="submit"><?= $isEdit ? 'Salvar alteracoes' : 'Cadastrar paciente' ?></button>
