...
<nav class="pac-tabs" aria-label="Seções da ficha do paciente">
    <?php foreach ($abas as $key => $label): ?>
        <a class="pac-tabs__link<?= ($aba ?? '') === $key ? ' pac-tabs__link--active' : '' ?>" href="<?= e($tabUrl($key)) ?>">
            <?= e($label) ?>
        </a>
    <?php endforeach; ?>
</nav>
...
<section class="panel">
    ...
    <div class="form-grid">
        <div class="info-card span-2">
            ...
            <?= e($valor($record['diagnostico'] ?? $record['diagnostico_principal'] ?? '')) ?>
        </div>

        <div class="info-card">
            <strong>CID Principal</strong><br>
            <?= e($valor($record['cid_principal'] ?? '')) ?>
        </div>

        <div class="info-card">
            <strong>Alergias</strong><br>
            <?= e($valor($record['alergias'] ?? '')) ?>
        </div>

        <div class="info-card">
            <strong>Mobilidade</strong><br>
            <?= e($valor($record['mobilidade'] ?? '')) ?>
        </div>

        <div class="info-card">
            <strong>Estado cognitivo base</strong><br>
            <?= e($valor($record['estado_cognitivo_base'] ?? '')) ?>
        </div>

        <div class="info-card">
            <strong>Oxigênio</strong><br>
            <?= $badge($record['usa_oxigenio'] ?? 'Não') ?>
        </div>

        <div class="info-card">
            <strong>Traqueostomia</strong><br>
            <?= $badge($record['traqueostomia'] ?? 'Não') ?>
        </div>

        <div class="info-card">
            <strong>GTT</strong><br>
            <?= $badge($record['gtt'] ?? $record['gastrostomia'] ?? 'Não') ?>
        </div>

        <div class="info-card">
            <strong>SNE</strong><br>
            <?= $badge($record['sne'] ?? 'Não') ?>
        </div>
        ...
        <div class="info-card span-2">
            ...
            <?= nl2br(e($valor($record['observacoes_clinicas'] ?? ''))) ?>
        </div>
    </div>
</section>
